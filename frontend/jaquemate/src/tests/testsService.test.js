...
describe("Usuarios - registrarUsuario", () => {
  ...
  it("lanza error 409", async () => {
    ...
    await expect(registrarUsuario({})).rejects.toThrow(
...
describe("Perfil - obtenerPerfilPorUsuario", () => {
  ...
  it("404 no encontrado", async () => {
    ...
    await expect(obtenerPerfilPorUsuario("x")).rejects.toThrow(
...
describe("Perfil - obtenerPerfilPorEmail", () => {
  ...
  it("404 email no encontrado", async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 404 });
    await expect(obtenerPerfilPorEmail("x")).rejects.toThrow(
      "Usuario no encontrado"
    );
  });
});
...
describe("Perfil - obtenerPerfilPorId", () => {
  ...
  it("404 id no encontrado", async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 404 });
    await expect(obtenerPerfilPorId(10)).rejects.toThrow(
      "Usuario no encontrado"
    );
  });
});
...
describe("Verificar usuario", () => {
  ...
  it("error HTTP genérico", async () => {
    ...
    await expect(verificarExisteUsuario("x")).rejects.toThrow(
      "Error HTTP: 500"
    );
  });
});
...
describe("Jugadas - obtenerJugadasPorUsuarioId", () => {
  ...
  it("error HTTP", async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 500 });
    await expect(obtenerJugadasPorUsuarioId(1)).rejects.toThrow(
      "Error HTTP: 500"
    );
  });
});
...
describe("Jugadas - obtenerJugadasPorNombre", () => {
  ...
  it("error HTTP", async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 500 });
    await expect(obtenerJugadasPorNombre("pepe")).rejects.toThrow(
      "Error HTTP: 500"
    );
  });
});
...
describe("Jugadas - obtenerJugadaPorId", () => {
  ...
  it("404 jugada no encontrada", async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 404 });
    await expect(obtenerJugadaPorId(99)).rejects.toThrow(
      "Jugada no encontrada"
    );
  });
});
...
describe("Jugadas - importarJugadasCSV", () => {
  ...
  it("error HTTP", async () => {
    ...
    await expect(importarJugadasCSV(fakeFile, 1)).rejects.toThrow(
      "Error HTTP: 500"
    );
  });
});
...
describe("Jugadas - confirmarImportacionJugadas", () => {
  ...
  it("error HTTP", async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 500 });
    await expect(confirmarImportacionJugadas(1, [])).rejects.toThrow(
      "Error HTTP: 500"
    );
  });
});
...
describe("Actualizar perfil", () => {
  ...
  it("404 no encontrado", async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 404 });
    await expect(actualizarPerfil(1, {})).rejects.toThrow(
      "Usuario no encontrado"
    );
  });
